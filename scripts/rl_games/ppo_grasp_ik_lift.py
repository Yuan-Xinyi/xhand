...
grasp pose.  We then measure how many latched grasps IK converts into strict 20 cm successes.
...
def main() -> None:
    ...
    # Per-env controller state.  phase: 0 = policy grasp, 1 = IK lift.
    ...
    # Hand each LIFT env its frozen joint target by overriding dof_targets after the normal
    # action pipeline runs (which also refreshes self.actions for the observation).
    ...
    for step in range(total_steps):
        ...
        if bool(lift_mask.any()):
            ...
            # Distal grip servo: tighten a fingertip toward grip_force_target, back off past the
            # limit, staying within a bounded band above the frozen grasp pose.  Holds contact as
            # the arm lifts so the tool does not slide out of a frozen hand.
        ...
        # --- strict success accounting (lift phase only) ---
        slow = (
    ...
    metrics = {
        ...
        "grasped_count": grasped,
        "grasped_rate": grasped / n,
        "ik_lift_success_count": lifted,
        "ik_lift_success_rate": lifted / n,
        "recovery_of_grasps": (lifted / grasped) if grasped else 0.0,
        "grasp_step": _summary(grasp_step[ever_grasped].float()) if grasped else None,
        "success_step": _summary(success_step[success].float()) if lifted else None,
    ...
    print(
        f"PPO grasp -> IK lift: grasped={grasped}/{n} ({grasped/n*100:.1f}%); "
        f"IK-lifted={lifted}/{n} ({lifted/n*100:.1f}%); "
