#!/usr/bin/env python3
"""Collect Candidate41's public-arm-ramp development A/B artifacts.

Both assignments execute one exact common Candidate39 route.  Candidate41
...
import candidate41_public_arm_ramp_episode as artifact_contract
import candidate41_public_arm_ramp as verifier
...
TOKEN_ACTION_DIM = 9
PUBLIC_ARM_RAMP_DENOMINATOR = 15.0
EXPECTED_V6_SHA256 = {
...
EXTRA_SOURCE_FILES = tuple(
    sorted(
        ...
        | {
            ...
            # this edge from its source closure; Candidate41 preregisters it.
            "scripts/flashsac/collect_candidate39_episode_residual_ab.py",
            "scripts/flashsac/candidate39_episode_residual.py",
            # Candidate41's supervisor and artifact contract reuse these
...
CANDIDATE41_INVOCATION_SEQUENCE = (
    (340, "b", 8),
    (341, "a", 64),
    (341, "b", 64),
    (342, "b", 64),
...
CANDIDATE41_RUN_ORDER = tuple(
    (seed, replicate) for seed, replicate, num_envs in CANDIDATE41_INVOCATION_SEQUENCE
    if num_envs == 64
)
PREREGISTRATION_TAG = "pick-tool-candidate41-public-arm-ramp-plan-v1-20260722"
COLLECTION_SEAL_TAG = "pick-tool-candidate41-public-arm-ramp-validation-v1-20260722"
ATTEMPT_RECEIPT_KIND = "pick_tool_candidate41_public_arm_ramp_attempt_v1"
...
def _require_exact(name: str, actual: Any, expected: Any) -> None:
    ...
    if not valid:
        raise ValueError(f"Candidate41 requires exact {name}={expected!r}")
...
def reconstruct_public_arm_ramp(
    *,
    ...
    """Rebuild Candidate41's public clock and effective arm multiplier.
    ...
    fraction = stable_count_before.to(dtype=torch.float32) / denominator
...
def validate_collection_seal_name_only_diff(
    ...
    if completed.returncode != 0:
        raise ValueError("cannot audit Candidate41 implementation-to-seal diff")
    names = tuple(line for line in completed.stdout.splitlines() if line)
    if names != (artifact_contract.VALIDATION_PLAN,):
        raise ValueError(
            "Candidate41 collection seal may differ from implementation only at the plan"
        )
...
def _protected_plan_contract(plan: Mapping[str, Any], *, pristine: bool) -> dict[str, Any]:
    ...
    try:
        ...
        raise ValueError("Candidate41 plan is not strict JSON") from error
    expected_status = (
        "preregistered_before_implementation_or_simulator_evidence"
        if pristine
        else artifact_contract.SEALED_PLAN_STATUS
    )
    if strict.get("status") != expected_status:
        raise ValueError("Candidate41 plan status is inconsistent with its seal phase")
    receipt = strict.get("preregistration_receipt")
    if not isinstance(receipt, dict):
        raise ValueError("Candidate41 plan omitted its preregistration receipt")
    if pristine and (
        receipt.get("commit") is not None or receipt.get("sha256") is not None
    ):
        raise ValueError("pristine Candidate41 plan already contained post-hoc receipts")
    if pristine:
        if strict.get("implementation_seal") is not None or strict.get(
            "collection_seal"
        ) is not None:
            raise ValueError("pristine Candidate41 plan already contained a later seal")
    else:
        if not isinstance(strict.get("implementation_seal"), dict) or not isinstance(
            strict.get("collection_seal"), dict
        ):
            raise ValueError("final Candidate41 plan omitted a required seal")
...
def validate_preregistration_protected_fields(
    ...
    if final != pristine:
        raise ValueError("Candidate41 final plan changed a preregistered protected field")
...
def require_exact_collection_head(root: Path, plan: Mapping[str, Any]) -> str:
    ...
    if not isinstance(collection, Mapping) or not isinstance(
        ...
        raise ValueError("Candidate41 plan omitted collection/preregistration authority")
    if collection.get("tag") != COLLECTION_SEAL_TAG:
        raise ValueError("Candidate41 collection-seal tag changed")
    if preregistration.get("tag") != PREREGISTRATION_TAG:
        raise ValueError("Candidate41 preregistration tag changed")
    ...
    if head != collection_commit:
        raise ValueError("HEAD must equal the separately sealed Candidate41 collection commit")
    declared_preregistration = preregistration.get("commit")
    if declared_preregistration != preregistration_commit:
        raise ValueError("Candidate41 preregistration receipt disagrees with its tag")
    ...
    if not isinstance(pristine_sha, str) or len(pristine_sha) != 64:
        raise ValueError("Candidate41 pristine plan SHA256 is absent")
    if _git_blob_sha256(
        root, commit=preregistration_commit, relative=artifact_contract.VALIDATION_PLAN
    ) != pristine_sha:
        raise ValueError("Candidate41 pristine preregistration plan changed")
    try:
        ...
        raise ValueError("Candidate41 pristine preregistration plan is not JSON") from error
    if not isinstance(pristine_plan, dict):
        raise ValueError("Candidate41 pristine preregistration plan is not an object")
...
def validate_all_submodules_clean(root: Path) -> None:
    ...
    if superproject.returncode != 0 or superproject.stdout.strip():
        raise ValueError("Candidate41 requires a clean superproject worktree")
    ...
    if status.returncode != 0:
        raise ValueError("cannot audit Candidate41 recursive submodules")
    for line in status.stdout.splitlines():
        if line and line[0] != " ":
            raise ValueError("Candidate41 submodule is missing or at an unregistered commit")
    ...
    if dirty.returncode != 0:
        raise ValueError("Candidate41 requires every recursive submodule to be clean")
...
def _validate_smoke_prerequisite(root: Path) -> tuple[str, str]:
    artifact_path = _regular_file(
        ...
        label="canonical Candidate41 smoke artifact",
    )
    report_path = _regular_file(
        root / artifact_contract.SMOKE_REPORT_PATH,
        label="canonical Candidate41 smoke report",
    )
    ...
    try:
        ...
        raise ValueError("cannot load the canonical Candidate41 smoke pair") from error
    ...
    if report["artifact_sha256"] != artifact_sha:
        raise ValueError("Candidate41 smoke report does not bind its artifact bytes")
    if Path(report["artifact_output"]).resolve() != artifact_path.resolve():
        raise ValueError("Candidate41 smoke report points to a non-canonical artifact")
    meta, episodes = checked["metadata"], checked["episodes"]
    if (meta["seed"], meta["replicate"], meta["num_envs"]) != (340, "b", 8):
        raise ValueError("Candidate41 smoke run identity changed")
    if meta["runtime_asset_sha256"] != RUNTIME_ASSET_EXPECTED_SHA256:
        raise ValueError("Candidate41 smoke runtime assets changed")
    treatment_eligible = episodes["treatment"] & episodes["eligible"]
    if not bool(treatment_eligible.any()):
        raise ValueError("Candidate41 smoke did not exercise a treatment overlay")
    if not bool((episodes["partial_authority_rows"][treatment_eligible] > 0).any()):
        raise ValueError("Candidate41 smoke did not exercise partial arm authority")
    if bool(episodes["new_abs_action_ge_0999"].any()):
        raise ValueError("Candidate41 smoke failed an action-algebra prerequisite")
    return artifact_sha, report_sha


def _validate_causal_receipts(root: Path) -> dict[str, str]:
    receipts = (
        (artifact_contract.CANDIDATE40_RESULT, artifact_contract.CANDIDATE40_RESULT_SHA256),
        (artifact_contract.CANDIDATE40_REPORT, artifact_contract.CANDIDATE40_REPORT_SHA256),
        (
    ...
    result = loaded[artifact_contract.CANDIDATE40_RESULT]
    report = loaded[artifact_contract.CANDIDATE40_REPORT]
    manifest = loaded[artifact_contract.FIXED_DIRECTION_MANIFEST]
    if (
        result.get("kind")
        != "pick_tool_candidate40_verified_arm_development_validation_result_v1"
        or result.get("status") != "sealed_rejected_before_formal_screen"
        or not isinstance(result.get("decision"), dict)
        or result["decision"].get("analyzer_decision") != "reject_candidate40"
        or result["decision"].get("all_gates_pass") is not False
    ):
        raise ValueError("Candidate40 result semantic identity changed")
    if (
        report.get("kind")
        != "pick_tool_candidate40_verified_arm_development_validation_v1"
        or report.get("status") != "complete"
        or report.get("decision") != "reject_candidate40"
    ):
        raise ValueError("Candidate40 combined report semantic identity changed")
...
def _canonical_dev_stem(root: Path, seed: int, replicate: str) -> Path:
    return (
        root
        / f"logs/flashsac/pick_tool/53_c41_public_arm_ramp_dev_s{seed}_{replicate}/trial"
    ).resolve()
...
def _canonical_invocation_stem(
    ...
    if (seed, replicate, num_envs) == CANDIDATE41_INVOCATION_SEQUENCE[0]:
        return (root / artifact_contract.SMOKE_ARTIFACT_PATH).with_suffix("").resolve()
    return _canonical_dev_stem(root, seed, replicate)


def _attempt_receipt_paths(stem: Path) -> tuple[Path, ...]:
    expression = re.compile(
        rf"^{re.escape(stem.name)}\.failed_attempt_([0-9]{{3}})\.json$"
    )
    found: list[tuple[int, Path]] = []
    for path in stem.parent.glob(f"{stem.name}.failed_attempt_*.json"):
        match = expression.fullmatch(path.name)
        if match is None or path.is_symlink() or not path.is_file():
            raise ValueError(f"invalid Candidate41 attempt receipt path: {path}")
        found.append((int(match.group(1)), path.resolve()))
    found.sort()
    if [number for number, _ in found] != list(range(1, len(found) + 1)):
        raise ValueError("Candidate41 failure receipt numbering is not contiguous")
...
def _argument_receipt(args: argparse.Namespace, output_stem: Path) -> dict[str, Any]:
    ...
    receipt = {
        name: _canonical_argument_value(name, value)
        for name, value in sorted(vars(args).items())
        if not name.startswith("_candidate41_")
    }
...
ATTEMPT_RECEIPT_FIELDS = {
    "kind", "status", "attempt", "first_env_step_invoked", "retry_permitted",
    "seed", "replicate", "num_envs", "canonical_artifact_output",
    "canonical_report_output", "collection_commit", "assignment_mask_sha256",
    "source_manifest_sha256", "checkpoint_manifest_sha256", "argument_receipt",
    "error_type", "error", "traceback",
}


def _load_attempt_receipts(
    stem: Path,
    *,
    seed: int,
    replicate: str,
    ...
    for index, path in enumerate(_attempt_receipt_paths(stem), start=1):
        try:
            value = json.loads(path.read_text(encoding="utf-8"))
        except (OSError, json.JSONDecodeError) as error:
            raise ValueError(f"cannot read Candidate41 attempt receipt {path}") from error
        if not isinstance(value, dict) or set(value) != ATTEMPT_RECEIPT_FIELDS:
            raise ValueError(f"Candidate41 attempt receipt schema changed: {path}")
        expected = {
            "kind": ATTEMPT_RECEIPT_KIND,
            "status": "failed",
            "attempt": index,
            "retry_permitted": not bool(value.get("first_env_step_invoked")),
            "seed": seed,
            "replicate": replicate,
            "num_envs": num_envs,
        ...
        if type(value["first_env_step_invoked"]) is not bool:
            raise ValueError("Candidate41 attempt boundary flag must be bool")
        for name, expected_value in expected.items():
            if value.get(name) != expected_value or type(value.get(name)) is not type(
                expected_value
            ):
                raise ValueError(f"Candidate41 attempt authority changed at {name}")
        for name in ("error_type", "error", "traceback"):
            if not isinstance(value[name], str):
                raise ValueError(f"Candidate41 attempt receipt {name} must be text")
...
def _validate_exact_invocation_attempt(
    ...
    *,
    ...
    if identity not in CANDIDATE41_INVOCATION_SEQUENCE:
        raise ValueError("invocation is outside Candidate41's exact five-entry sequence")
    current_index = CANDIDATE41_INVOCATION_SEQUENCE.index(identity)
    for index, (item_seed, item_replicate, item_envs) in enumerate(
        ...
        item_argument_receipt = _argument_receipt_for_identity(
            argument_receipt,
            seed=item_seed,
            replicate=item_replicate,
            num_envs=item_envs,
            output_stem=stem,
        )
        item_assignment_sha = verifier.assignment_mask_sha256(
        ...
        item_checkpoints = dict(checkpoint_sha256)
        if item_envs == 8:
            item_checkpoints.pop(artifact_contract.SMOKE_ARTIFACT_PATH, None)
            item_checkpoints.pop(artifact_contract.SMOKE_REPORT_PATH, None)
        elif index <= current_index and (
            artifact_contract.SMOKE_ARTIFACT_PATH not in item_checkpoints
            or artifact_contract.SMOKE_REPORT_PATH not in item_checkpoints
        ):
            raise ValueError("development invocation omitted sealed smoke receipts")
        item_checkpoint_manifest_sha = _json_sha256(item_checkpoints)
        artifact_owned = _owned(Path(f"{stem}.pt"))
        report_owned = _owned(Path(f"{stem}.json"))
        receipts = _attempt_receipt_paths(stem)
        if index < current_index:
            if not artifact_owned or not report_owned:
                raise ValueError(
                    f"Candidate41 invocation prerequisite is incomplete: "
                    f"{item_seed}{item_replicate}"
                )
            prior = _load_attempt_receipts(
                stem,
                seed=item_seed,
                replicate=item_replicate,
                num_envs=item_envs,
                collection_commit=collection_commit,
                assignment_mask_sha256=item_assignment_sha,
                source_manifest_sha256=source_manifest_sha256,
                checkpoint_manifest_sha256=item_checkpoint_manifest_sha,
                argument_receipt=item_argument_receipt,
            )
            # A post-step failure is authoritative rejection even if someone
            # later materialized canonical-looking success outputs.
            if any(item["first_env_step_invoked"] for item in prior):
                raise ValueError(
                    "Candidate41 was rejected by a prior post-step failure"
                )
        elif index == current_index:
            if artifact_owned or report_owned:
                raise FileExistsError(f"Candidate41 canonical output already exists: {stem}")
            current = _load_attempt_receipts(
                stem,
                seed=seed,
                replicate=replicate,
                num_envs=num_envs,
            ...
            if item_assignment_sha != assignment_mask_sha256:
                raise RuntimeError("current Candidate41 assignment receipt changed")
            if any(item["first_env_step_invoked"] for item in current):
                raise ValueError(
                    "Candidate41 current sequence position already failed after env.step"
                )
        elif artifact_owned or report_owned or receipts:
            raise ValueError(
                f"later Candidate41 invocation already exists out of order: "
                f"{item_seed}{item_replicate}"
            )
...
def _validate_prior_authority(
    ...
    *,
    ...
    if metadata.get("source_sha256") != dict(source_sha256):
        raise ValueError("prior Candidate41 artifact used a different source authority")
    if (
        metadata.get("smoke_artifact_sha256") != smoke_artifact_sha256
        or metadata.get("smoke_report_sha256") != smoke_report_sha256
    ):
        raise ValueError("prior Candidate41 artifact used different smoke authority")
    git = metadata.get("git")
    if not isinstance(git, Mapping) or git.get("commit") != collection_commit:
        raise ValueError("prior Candidate41 artifact used a different collection commit")
    if metadata.get("runtime_asset_sha256") != RUNTIME_ASSET_EXPECTED_SHA256:
        raise ValueError("prior Candidate41 artifact used different runtime assets")
...
def _validate_published_pair(
    ...
    *,
    ...
    if report["artifact_sha256"] != digest or Path(
        ...
        raise ValueError("prior Candidate41 report does not bind canonical artifact bytes")
    ...
    if (metadata["seed"], metadata["replicate"], metadata["num_envs"]) != (
        ...
        raise ValueError("prior Candidate41 development run identity changed")
...
def _validate_run_order(
    ...
    *,
    ...
    if identity not in CANDIDATE41_RUN_ORDER:
        raise ValueError("run is outside the sealed Candidate41 development order")
    index = CANDIDATE41_RUN_ORDER.index(identity)
    ...
    for prior_seed, prior_replicate in CANDIDATE41_RUN_ORDER[:index]:
        prior_stem = _canonical_dev_stem(root, prior_seed, prior_replicate)
        try:
            artifact_path = _regular_file(
                Path(f"{prior_stem}.pt"),
                label=f"prior Candidate41 artifact {prior_seed}{prior_replicate}",
            )
            report_path = _regular_file(
                Path(f"{prior_stem}.json"),
                label=f"prior Candidate41 report {prior_seed}{prior_replicate}",
            )
        except FileNotFoundError as error:
            raise ValueError(
                f"Candidate41 run order prerequisite is missing: {prior_seed}{prior_replicate}"
            ) from error
    ...
    for later_seed, later_replicate in CANDIDATE41_RUN_ORDER[index + 1 :]:
        ...
        if _owned(Path(f"{later_stem}.pt")) or _owned(Path(f"{later_stem}.json")):
            raise ValueError(
                f"later Candidate41 output already exists out of order: {later_seed}{later_replicate}"
            )
...
def validate_runtime_source_count(
    ...
    try:
        ...
        raise ValueError("Candidate41 seal omitted the runtime source count") from error
    if type(declared) is not int or declared != len(source_sha256):
        raise ValueError(
            "Candidate41 static-audit runtime source count does not match "
...
def build_spec(args: argparse.Namespace) -> CollectionSpec:
    ...
    if args.num_envs == 8 and (args.seed, args.replicate) != (340, "b"):
        raise ValueError("the only Candidate41 smoke is exact run 340b/8-env")
    if args.num_envs == 64 and args.seed not in {341, 342}:
        raise ValueError("Candidate41 development uses only seeds 341 and 342")
    ...
    if getattr(args, "kit_args", None) != artifact_contract.KIT_ARGS:
        raise ValueError(
            f"Candidate41 requires exact --kit_args={artifact_contract.KIT_ARGS!r}"
        )
    ...
    for label, (actual, expected) in canonical_inputs.items():
        if actual.resolve() != expected.resolve():
            raise ValueError(f"Candidate41 {label} is not the registered canonical path")
    ...
    if checkpoint_sha256["v6"] != EXPECTED_V6_SHA256:
        raise ValueError("Candidate41 requires the exact sealed V6 checkpoint")
    if checkpoint_sha256["search"] != artifact_contract.SEARCH_CHECKPOINT_SHA256:
        raise ValueError("Candidate41 requires the exact sealed SEARCH checkpoint")
    if checkpoint_sha256["fixed_direction"] != artifact_contract.FIXED_DIRECTION_SHA256:
        raise ValueError("Candidate41 requires the exact sealed fixed_z payload")
    ...
    if (
        ...
        raise ValueError(
            "Candidate41 pre-launch Git authority is dirty, detached, or changed"
        )
    ...
    for output in (artifact_output, report_output):
        if _owned(output):
            raise FileExistsError(f"Candidate41 output already exists: {output}")
    ...
    if not torch.equal(treatment, expected_from_rank):
        raise RuntimeError("Candidate41 assignment mask disagrees with assignment rank")
    if int(treatment.sum()) != args.num_envs // 2:
        raise RuntimeError("Candidate41 assignment is not exactly balanced")
...
def _current_hashes(spec: CollectionSpec) -> dict[str, Mapping[str, str] | str]:
    ...
    for relative in (
        artifact_contract.CANDIDATE40_RESULT,
        artifact_contract.CANDIDATE40_REPORT,
        artifact_contract.FIXED_DIRECTION_MANIFEST,
    ):
...
def validate_post_application_authority(spec: CollectionSpec) -> None:
    ...
    if source != dict(spec.source_sha256) or _current_hashes(spec) != dict(
        ...
        raise RuntimeError("Candidate41 source or checkpoint changed after collection")
    if runtime_assets != RUNTIME_ASSET_EXPECTED_SHA256:
        raise RuntimeError("Candidate41 runtime asset changed after collection")
    git_paths = tuple(
        sorted(
            set(source)
    ...
    if (
        ...
        raise RuntimeError("Candidate41 Git authority changed after collection")
...
def run_collection(
    spec: CollectionSpec, *, device_string: str, progress: AttemptProgress | None = None
) -> tuple[dict[str, Any], dict[str, Any]]:
    if progress is None:
        progress = AttemptProgress()
    device = torch.device(device_string)
    if device.type != "cuda" or not torch.cuda.is_available():
        raise RuntimeError(f"Candidate41 collection requires CUDA, got {device}")
    ...
    if not torch.equal(
        ...
        raise RuntimeError("pre-launch Candidate41 assignment changed")
    if verifier.assignment_mask_sha256(spec.treatment) != spec.assignment_mask_sha256:
        raise RuntimeError("pre-launch Candidate41 assignment receipt changed")
    ...
    if not torch.equal(fixed_payload["fixed_z"], spec.fixed_z):
        raise RuntimeError("pre-launch Candidate41 fixed_z changed")
    ...
    if source_before != dict(spec.source_sha256):
        raise RuntimeError("Candidate41 source authority changed after pre-launch freeze")
    ...
    if git_before["source_files_dirty"] or git_before["flashsac_dirty"]:
        raise RuntimeError("Candidate41 collection requires committed clean sources")
    if (
        ...
        raise RuntimeError("Candidate41 Git authority changed after pre-launch freeze")
    ...
    try:
        ...
        if bool(initial_truth.grasped.any()):
            raise RuntimeError("Candidate41 reset unexpectedly begins grasped")
        ...
        while not tracker.complete and vector_steps < artifact_contract.MAX_EPISODE_ACTIONS:
            ...
            if bool((trigger & triggered).any()):
                raise RuntimeError("a Candidate41 slot triggered twice")
            triggered |= trigger
            ...
            if not torch.equal(residual.eligible, window.active):
                raise RuntimeError("fixed residual assignment incorrectly used Candidate41 arm")
            ...
            if executed is None or not torch.equal(executed, requested_action):
                raise RuntimeError("adapter executed a different Candidate41 request")
        ...
        if not tracker.complete or len(tracker.records) != spec.num_envs:
            raise RuntimeError(
                f"Candidate41 completed {len(tracker.records)}/{spec.num_envs} first episodes"
            )
        ...
        if set(records_by_slot) != set(range(spec.num_envs)):
            raise RuntimeError("Candidate41 tracker omitted an environment slot")
        ...
        if any(int(record["slot_episode_index"]) != 0 for record in ordered):
            raise RuntimeError("Candidate41 artifact contains a post-reset episode")
        ...
        if (
            ...
            raise RuntimeError("Candidate41 source/runtime/checkpoint changed during collection")
...
def parse_args() -> tuple[argparse.Namespace, CollectionSpec, Any]:
    ...
    # AppLauncher construction is deliberately owned by main() so a launch
    # failure after authority was frozen receives a durable pre-step receipt.
    return args, spec, AppLauncher


def publish_failure_attempt(
    spec: CollectionSpec, error: BaseException, progress: AttemptProgress
) -> Path:
    prior = _attempt_receipt_paths(spec.output_stem)
    attempt = len(prior) + 1
    payload = {
        "kind": ATTEMPT_RECEIPT_KIND,
        "status": "failed",
        "attempt": attempt,
        "first_env_step_invoked": progress.first_env_step_invoked,
        "retry_permitted": not progress.first_env_step_invoked,
        "seed": spec.seed,
        "replicate": spec.replicate,
        "num_envs": spec.num_envs,
        ...
        "checkpoint_manifest_sha256": _json_sha256(spec.checkpoint_sha256),
...
def main() -> None:
    args, spec, launcher_type = parse_args()
    progress = AttemptProgress()
    simulation_app = None
    try:
        ...
        # Application shutdown belongs to the authoritative attempt.  Close it
        # before publishing success so a post-step shutdown failure cannot
        # leave a canonical-looking successful pair without a rejection receipt.
        app_to_close = simulation_app
        simulation_app = None
        app_to_close.close()
        validate_post_application_authority(spec)
        ...
        if simulation_app is not None:
            try:
                ...
                # The original failure already determines the campaign result;
                # a cleanup failure cannot make the attempt retryable.
                pass
        failure = publish_failure_attempt(spec, error, progress)
        print(f"[candidate41-public-arm-ramp-ab] failure recorded at {failure}", flush=True)
        raise
    # The canonical pair is now durable and defines a successful attempt.
    # A downstream stdout/BrokenPipe failure must not manufacture a conflicting
    # post-step failure receipt beside valid evidence.
    print(
        "[candidate41-public-arm-ramp-ab] "
