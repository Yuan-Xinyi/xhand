#!/usr/bin/env python3
"""CPU corruption tests and synthetic fixtures for Candidate41 artifacts."""
...
def _metadata(*, seed: int, replicate: str, num_envs: int) -> dict:
    sources = {"scripts/flashsac/fake_candidate41.py": "a" * 64}
    assets = {"/tmp/fake_candidate41.usd": "b" * 64}
...
def _sealed_plan_fixture() -> dict:
    ...
    plan["implementation_seal"] = {
        ...
        "source_sha256": {
            relative: contract.sha256_file(root / relative)
            for relative in contract.IMPLEMENTATION_SOURCE_FILES
        },
...
def _payload(
    *, seed: int = 340, replicate: str = "b", num_envs: int = 8
) -> tuple[
    dict,
...
def test_plan_identity_fail_closed() -> None:
    ...
    bad["candidate41_intervention_contract"]["name"] = "changed"
    mutations.append(bad)
    bad = copy.deepcopy(plan)
...
def test_metadata_implementation_commit_is_bound_when_sealed() -> None:
    metadata = _metadata(seed=340, replicate="b", num_envs=8)
    plan = contract.validate_sealed_plan(require_sealed=False)
    fake_sealed = {
        **plan,
        "implementation_seal": {
...
def test_complements_and_no_clobber_publication() -> None:
    left = _artifact(seed=341, replicate="a", num_envs=64)
    right = _artifact(seed=341, replicate="b", num_envs=64)
...
def main() -> None:
    ...
    test_complements_and_no_clobber_publication()
    print("candidate41_public_arm_ramp_episode tests passed")
