#!/usr/bin/env python3
"""Analyze the sealed Candidate41 public-arm-ramp development A/B.
...
REPORT_KIND = "pick_tool_candidate41_public_arm_ramp_development_validation_v1"
FORMAT_VERSION = 1
SEEDS = (341, 342)
REPLICATES = ("a", "b")
RUN_ORDER = ((341, "a"), (341, "b"), (342, "b"), (342, "a"))
INVOCATION_SEQUENCE = (
    (340, "b", 8),
    (341, "a", 64),
    (341, "b", 64),
...
INPUT_PATTERN = (
    "logs/flashsac/pick_tool/53_c41_public_arm_ramp_dev_s{seed}_{replicate}/trial.pt"
)
REPORT_OUTPUT = (
    "logs/flashsac/pick_tool/53_c41_public_arm_ramp_development_validation.json"
)
...
def _attempt_receipt_paths(stem: Path) -> tuple[Path, ...]:
    ...
    if [number for number, _ in found] != list(range(1, len(found) + 1)):
        raise ValueError("Candidate41 attempt receipt numbering is not contiguous")
...
def _sha_receipt(value: Any, name: str, *, length: int) -> str:
    if (
        ...
        raise ValueError(f"Candidate41 {name} is not a lowercase hex receipt")
    return value


def validate_attempt_receipts(
    root: Path, *, collection_commit: str
) -> list[dict[str, Any]]:
    """Reject any authoritative post-step failure in all five namespaces."""
    ...
    for seed, replicate, num_envs in INVOCATION_SEQUENCE:
        ...
        paths = _attempt_receipt_paths(stem)
        for attempt, path in enumerate(paths, start=1):
            try:
                ...
                raise ValueError(
                    f"cannot read Candidate41 attempt receipt {path}"
                ) from error
            receipt_sha256 = _sha256_bytes(raw)
            if not isinstance(value, dict) or set(value) != ATTEMPT_RECEIPT_FIELDS:
                raise ValueError("Candidate41 attempt receipt schema changed")
            if (
                value["kind"] != ATTEMPT_RECEIPT_KIND
                or value["status"] != "failed"
                or type(value["attempt"]) is not int
                or value["attempt"] != attempt
                or type(value["first_env_step_invoked"]) is not bool
                or type(value["retry_permitted"]) is not bool
                or value["retry_permitted"] is value["first_env_step_invoked"]
                or type(value["seed"]) is not int
                or type(value["num_envs"]) is not int
                or (value["seed"], value["replicate"], value["num_envs"])
                != (seed, replicate, num_envs)
            ):
                raise ValueError("Candidate41 attempt receipt identity changed")
            if (
                value["canonical_artifact_output"] != str(Path(f"{stem}.pt"))
                or value["canonical_report_output"] != str(Path(f"{stem}.json"))
                or value["collection_commit"] != collection_commit
            ):
                raise ValueError("Candidate41 attempt receipt authority changed")
            ...
            if value["assignment_mask_sha256"] != expected_assignment:
                raise ValueError("Candidate41 attempt assignment receipt changed")
            for name in (
            ...
            arguments = value["argument_receipt"]
            if not isinstance(arguments, dict) or (
                ...
                raise ValueError("Candidate41 attempt argument receipt changed")
            for name in ("error_type", "error", "traceback"):
                if not isinstance(value[name], str):
                    raise ValueError(f"Candidate41 attempt {name} must be text")
            current_authority = {
                name: value[name]
                for name in (
                    "collection_commit",
            ...
            if authority is None:
                ...
                raise ValueError("Candidate41 pre-step retry authority changed")
            summary.append(
                {
                    ...
                    "first_env_step_invoked": value["first_env_step_invoked"],
                    "retry_permitted": value["retry_permitted"],
                }
            )
            if value["first_env_step_invoked"]:
                raise ValueError(
                    "Candidate41 has an authoritative post-env.step failure receipt"
                )
    return summary
...
def _read_plan(
    ...
    path = _regular_file(root / artifact_contract.VALIDATION_PLAN, "Candidate41 plan")
    raw = path.read_bytes()
...
def registered_gate_thresholds(
    ...
    if not isinstance(gates, dict) or gates.get("all_must_pass") is not True:
        raise ValueError("Candidate41 plan omitted all-must-pass gates")
    values = {key: value for key, value in gates.items() if key != "all_must_pass"}
    if set(values) != set(GATE_NAMES):
        raise ValueError("Candidate41 registered gate set changed")
    return values
...
def validate_validation_artifacts(
    ...
    *,
    ...
    if set(artifacts) != set(RUN_ORDER):
        raise ValueError("Candidate41 fixed run set changed")
    ...
    for seed, replicate in RUN_ORDER:
        ...
        if (metadata["seed"], metadata["replicate"], metadata["num_envs"]) != (
            ...
            raise ValueError("Candidate41 artifact run identity changed")
        ...
        if not torch.equal(episodes["assignment_rank"], expected_rank):
            raise ValueError("Candidate41 independent assignment rank changed")
        if not torch.equal(episodes["treatment"], expected_mask):
            raise ValueError("Candidate41 independent treatment mask changed")
        common = _without_run_identity(metadata)
        if reference is None:
            reference = common
        elif common != reference:
            raise ValueError("Candidate41 artifacts do not share one authority")
...
def stable_transport_outcomes(
    ...
    for slot in range(count):
        ...
        if ages.numel() == 0 or not torch.equal(
            ...
            raise ValueError(f"eligible Candidate41 slot {slot} utility trace has a gap")
        ...
        if bool(episodes["success"][slot]):
            ...
            if terminal_age < 0:
                raise ValueError("Candidate41 success precedes eligibility")
...
def _ht_outcome(
    ...
    if treatment.shape != domain.shape or numeric.shape != domain.shape:
        raise ValueError("Candidate41 HT shapes differ")
    if not bool(torch.isfinite(numeric).all()):
        raise FloatingPointError("Candidate41 HT outcome is non-finite")
    ...
    if denominator <= 0 or treatment_rows <= 0 or control_rows <= 0:
        raise ValueError("a Candidate41 seed lacks eligible support in one arm")
...
def _paired_common_eligibility(
    ...
    return {
        ...
        "domain": "same seed/env_slot eligible before Candidate41 action in both replicates",
...
def _action_clock_audit(
    ...
    for seed, replicate in RUN_ORDER:
        ...
        if int((scale_bad | arm_bad).sum()) != row[
            ...
            raise ValueError("online and trace Candidate41 ramp audits disagree")
        if int(hand_bad.sum()) != row["hand_invariance_violations"]:
            raise ValueError("online and trace Candidate41 hand audits disagree")
        derived_partial = int((overlay & (ramp_scale > 0.0) & (ramp_scale < 1.0)).sum())
        if derived_partial != int(episodes["partial_authority_rows"].sum()):
            raise ValueError("online and trace Candidate41 partial-row counts disagree")
        ...
        if derived_new != row["any_new_abs_action_ge_0999"]:
            raise ValueError("online and trace Candidate41 saturation audits disagree")
...
def _build_gate_results(
    ...
    if set(specs) != set(GATE_NAMES) or set(thresholds) != set(GATE_NAMES):
        raise RuntimeError("not every Candidate41 gate has executable semantics")
    return {
...
def _authority_by_age_count(
    ...
    *,
    ...
    if (
        ...
        raise ValueError("Candidate41 authority deadline tensors differ")
    if not isinstance(maximum_inclusive_age, int) or isinstance(
        maximum_inclusive_age, bool
    ) or maximum_inclusive_age < 0:
        raise ValueError("Candidate41 authority deadline age is invalid")
...
def compute_validation(
    ...
    *,
    ...
    report = {
        ...
        "decision": "advance_to_formal_screen" if all_pass else "reject_candidate41",
...
def _read_artifact(
    ...
    raw = _regular_file(path, "Candidate41 artifact").read_bytes()
...
def _validate_registered_smoke(
    ...
    artifact_path = _regular_file(
        root / artifact_contract.SMOKE_ARTIFACT_PATH, "Candidate41 smoke artifact"
    )
    report_path = _regular_file(
        root / artifact_contract.SMOKE_REPORT_PATH, "Candidate41 smoke report"
    )
    ...
    artifact_contract.validate_report(report, artifact, published=True)
    digest = _sha256_bytes(raw)
    if report["artifact_sha256"] != digest:
        raise ValueError("Candidate41 smoke report does not bind artifact bytes")
    ...
    if violations or bool(artifact["episodes"]["new_abs_action_ge_0999"].any()):
        raise ValueError("Candidate41 smoke failed action/trace algebra")
    return {
        "run": "340b",
...
def _validate_collection_head(root: Path, plan: Mapping[str, Any]) -> str:
    ...
    if not isinstance(seal, Mapping) or seal.get("tag") != (
        "pick-tool-candidate41-public-arm-ramp-validation-v1-20260722"
    ):
    ...
    if head != tagged:
        raise ValueError("HEAD is not the exact Candidate41 collection seal")
    ...
    if prereg.get("commit") != prereg_commit:
        raise ValueError("Candidate41 preregistration tag/commit disagree")
    ancestor = subprocess.run(
        ("git", "merge-base", "--is-ancestor", prereg_commit, head), cwd=root,
        stdout=subprocess.PIPE, stderr=subprocess.PIPE,
    )
    if ancestor.returncode != 0:
        raise ValueError("Candidate41 collection does not descend from preregistration")
    ...
    if _sha256_bytes(pristine_raw) != prereg.get("sha256"):
        raise ValueError("Candidate41 pristine plan receipt changed")
    ...
    if normalized != pristine:
        raise ValueError("Candidate41 protected preregistration content changed")
    dirty = subprocess.run(
        ("git", "status", "--porcelain", "--untracked-files=all"), cwd=root,
        check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True,
    ).stdout
    if dirty:
        raise ValueError("Candidate41 collection/analyzer worktree is dirty")
    return head
...
def load_fixed_evidence(
    ...
    attempt_receipts = validate_attempt_receipts(
        root, collection_commit=collection_commit
    )
    smoke = _validate_registered_smoke(root)
    from collect_candidate41_public_arm_ramp_ab import source_fingerprints
    from collect_online_close_ab import runtime_asset_fingerprints
    ...
    for seed, replicate in RUN_ORDER:
        ...
        collector_report = json.loads(
            _regular_file(report_path, "Candidate41 collector report").read_text(
                encoding="utf-8"
            )
        )
        artifact_contract.validate_report(
            collector_report, artifact, published=True
        )
        if collector_report["artifact_sha256"] != digest or Path(
            collector_report["artifact_output"]
        ).resolve() != path.resolve():
            raise ValueError("Candidate41 collector report does not bind artifact")
        metadata = artifact["metadata"]
        if metadata["source_sha256"] != current_source:
            raise ValueError("Candidate41 current runtime source closure changed")
        if metadata["runtime_asset_sha256"] != current_runtime_assets:
            raise ValueError("Candidate41 current runtime assets changed")
        if metadata["validation_plan_sha256"] != plan_sha:
            raise ValueError("Candidate41 artifact used another final plan")
        if metadata["smoke_artifact_sha256"] != smoke["artifact_sha256"] or (
            metadata["smoke_report_sha256"] != smoke["report_sha256"]
        ):
            raise ValueError("Candidate41 artifact used another smoke authority")
        git = metadata.get("git")
        if not isinstance(git, Mapping) or git.get("commit") != collection_commit:
            raise ValueError("Candidate41 artifact used another collection commit")
    ...
    fixed = {
        ...
        "pre_step_failure_receipts": attempt_receipts,
...
def publish_json_no_clobber(payload: Mapping[str, Any], output: Path) -> None:
    ...
    if output.exists() or output.is_symlink():
        raise FileExistsError(f"refusing to overwrite Candidate41 report: {output}")
...
def main() -> None:
    ...
    print(
        "[candidate41-public-arm-ramp-validation] "
