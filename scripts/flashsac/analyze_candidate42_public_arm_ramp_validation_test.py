#!/usr/bin/env python3
"""CPU-only tests for Candidate42's fail-closed development analyzer."""
...
import analyze_candidate42_public_arm_ramp_validation as analyzer
from candidate42_public_arm_ramp_episode_test import _artifact as _base_artifact
from analyze_candidate42_public_arm_ramp_validation import (
    GATE_NAMES,
    RUN_ORDER,
    SEEDS,
    _action_clock_audit,
...
def test_stable_transport_requires_15_rows_and_success_suffix_starts_after_terminal() -> None:
    base = _base_artifact(seed=347, replicate="a", num_envs=64)
...
def test_linear_float32_action_audit_reconstructs_scale_arm_and_hand_exactly() -> None:
    ...
    unacknowledged[(347, "a")]["steps"]["row_authority_scale"][row] += 0.01
...
def test_atomic_no_clobber_publication() -> None:
    ...
    with tempfile.TemporaryDirectory() as directory:
        output = Path(directory) / "candidate42.json"
        fsynced: list[Path] = []
        real_fsync_directory = analyzer._fsync_directory

        def checked_fsync_directory(path: Path) -> None:
            assert output.is_file() and not output.is_symlink()
            fsynced.append(path)
            real_fsync_directory(path)

        with mock.patch.object(
            analyzer, "_fsync_directory", side_effect=checked_fsync_directory
        ):
...
def test_no_clobber_publication_surfaces_post_link_directory_fsync_failure() -> None:
    payload = {"decision": "fail_closed", "network_updates": 0}
    with tempfile.TemporaryDirectory() as directory:
        output = Path(directory) / "candidate42.json"
        ...
        with mock.patch.object(
            analyzer, "_fsync_directory", side_effect=fail_after_link
        ):
...
def _successful_namespace(
    ...
    *,
    ...
    assignment_mask_sha256: str | None = None,
) -> tuple[transaction.TransactionPaths, transaction.AttemptIdentity, dict]:
    """Build one real 00--60 committed namespace without Isaac imports."""

    stem.parent.mkdir(parents=True, exist_ok=True)
    paths = transaction.TransactionPaths.from_output_stem(stem)
    assignment = assignment_mask_sha256 or ramp.assignment_mask_sha256(
    ...
    checkpoint_sha = "b" * 64
    runtime_sha = "c" * 64
    canonical = paths.as_run_id_dict()
    run_metadata = {
        "seed": seed,
        "replicate": replicate,
        "num_envs": num_envs,
        "collection_commit": collection_commit,
        "assignment_mask_sha256": assignment,
        "source_manifest_sha256": source_sha,
        "checkpoint_manifest_sha256": checkpoint_sha,
        "runtime_asset_manifest_sha256": runtime_sha,
        "preregistration_tag_commit": "f" * 40,
        "canonical_artifact_output": canonical["artifact"],
        "canonical_report_output": canonical["report"],
        "canonical_commit_output": canonical["commit"],
        "argument_receipt": {
            "seed": seed,
            "replicate": replicate,
            "num_envs": num_envs,
    ...
    run_identity = {
        "collection_commit": collection_commit,
        "parent_argv": ["synthetic-candidate42-parent", str(stem)],
        "assignment_mask_sha256": assignment,
        "checkpoint_manifest_sha256": checkpoint_sha,
        "source_manifest_sha256": source_sha,
        "runtime_asset_manifest_sha256": runtime_sha,
    ...
    with closing(transaction.NamespaceLock.acquire(paths)) as lock:
        ...
        boundary = transaction.write_receipt(
            stage,
        ...
        payload = transaction.durable_stage_bytes(
            stage, transaction.STAGED_PAYLOAD, b"synthetic CPU weights-only artifact"
        )
        report_core = transaction.durable_stage_bytes(
            stage, transaction.STAGED_REPORT_CORE, b"{}\n"
        )
        ...
        close = transaction.write_receipt(
            stage,
        ...
        stdout = transaction.durable_stage_bytes(
            stage, transaction.STDOUT_LOG, b"synthetic stdout\n"
        )
        stderr = transaction.durable_stage_bytes(
            stage, transaction.STDERR_LOG, b"synthetic stderr\n"
        )
        child_exit = transaction.write_receipt(
            stage,
            transaction.CHILD_EXIT,
            identity,
            {
                "status": "observed",
                "raw_wait_status": 0,
        ...
        final = stage.write_bytes(
            transaction.FINAL_REPORT,
            transaction.pretty_json_bytes(final_report),
        )
        transaction.load_receipt(stage, transaction.FINAL_REPORT)
        ...
        committed = transaction.publish_verified_attempt(paths, identity, lock)
    return paths, identity, committed


def test_committed_namespace_requires_commit_and_rejects_terminal_70() -> None:
    collection_commit = "d" * 40
    with tempfile.TemporaryDirectory() as directory:
        root = Path(directory)
        stem = _canonical_invocation_stem(root, 346, "b", 8)
        paths, identity, _ = _successful_namespace(
            stem,
            seed=346,
            replicate="b",
            num_envs=8,
        ...
        accepted = validate_committed_invocation(
            root,
            seed=346,
            replicate="b",
            num_envs=8,
        ...
        assert accepted["identity"] == identity.as_dict()
        paths.commit.unlink()
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        with closing(transaction.NamespaceLock.acquire(paths)) as lock:
            transaction.record_parent_failure(
                ...
                identity,
                lock,
                reason="synthetic terminal conflict",
                raw_wait_status=0,
...
def test_committed_namespace_rejects_registered_identity_or_manifest_mismatch() -> None:
    collection_commit = "d" * 40
    with tempfile.TemporaryDirectory() as directory:
        root = Path(directory)
        stem = _canonical_invocation_stem(root, 346, "b", 8)
        _successful_namespace(
            stem,
            seed=347,
            replicate="b",
            num_envs=8,
            collection_commit=collection_commit,
            assignment_mask_sha256="e" * 64,
        )
        with pytest.raises(ValueError, match="run metadata changed"):
            validate_committed_invocation(
                root,
                seed=346,
                replicate="b",
...
def test_fixed_evidence_uses_candidate42_authority_helpers() -> None:
    source = inspect.getsource(analyzer.load_fixed_evidence)
    assert "from candidate42_public_arm_ramp_collection import (" in source
    assert "collect_candidate42_public_arm_ramp_ab import source_fingerprints" not in source
    from candidate42_public_arm_ramp_collection import (
        runtime_asset_fingerprints,
        source_fingerprints,
    )

    assert callable(source_fingerprints)
    assert callable(runtime_asset_fingerprints)
