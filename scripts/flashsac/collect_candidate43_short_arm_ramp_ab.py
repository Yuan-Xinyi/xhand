#!/usr/bin/env python3
"""Run Candidate43 attempts under a durable parent-owned transaction."""
...
_CHILD_BASENAME = "collect_candidate43_short_arm_ramp_child.py"
    ...
    *,
    ...
    if failure.exists():
        ...
        if failure_evidence.payload.get("retry_permitted") is not True:
            raise RuntimeError(
                "latest Candidate43 attempt is terminal and non-retryable"
            )
    ...
    if committed.exists() or canonical_commit.exists():
        ...
        raise RuntimeError("Candidate43 commit chain is missing 56_verified")
    if not failure.exists():
        ...
        if child_exit_exists:
            try:
                ...
                raise RuntimeError(
                    "invalid existing Candidate43 child-exit evidence"
                ) from error
        if not prespawn_failed and not child_exit_exists:
            ...
            try:
                ...
                try:
                    ...
                    raise RuntimeError(
                        "could not seal unsafe Candidate43 recovery"
                    ) from failure_error
                raise RuntimeError("unsafe unclassified Candidate43 recovery") from error
