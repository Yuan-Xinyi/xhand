#!/usr/bin/env python3
"""Pure public-state linear arm-authority ramp for Candidate41.
...
bit-exact.  The stability predicate and evidence-clock timing are delegated
to Candidate40's sealed implementation so Candidate41 changes only the
authority mapping.
"""
...
OBSERVATION_DIM = candidate40.OBSERVATION_DIM
ACTION_DIM = candidate40.ACTION_DIM
ARM_ACTION_DIM = candidate40.ARM_ACTION_DIM
HAND_ACTION_DIM = candidate40.HAND_ACTION_DIM

FORCE_STRENGTH_START = candidate40.FORCE_STRENGTH_START
FORCE_STRENGTH_STOP = candidate40.FORCE_STRENGTH_STOP
STRICT_WRAP_QUALITY_INDEX = candidate40.STRICT_WRAP_QUALITY_INDEX
HOLD_QUALITY_INDEX = candidate40.HOLD_QUALITY_INDEX
PUBLIC_LATCH_INDEX = candidate40.PUBLIC_LATCH_INDEX
GRASP_QUALITY_MIN = candidate40.GRASP_QUALITY_MIN
HOLD_QUALITY_MIN = candidate40.HOLD_QUALITY_MIN
PUBLIC_FORCE_STRENGTH_MAX = candidate40.PUBLIC_FORCE_STRENGTH_MAX
SAFE_FORCE_STRENGTH_MAX = candidate40.SAFE_FORCE_STRENGTH_MAX
SAFE_FORCE_LIMIT_N = candidate40.SAFE_FORCE_LIMIT_N
FORCE_SATURATION_N = candidate40.FORCE_SATURATION_N
PUBLIC_FORCE_COUNTER_CONTRACT = candidate40.PUBLIC_FORCE_COUNTER_CONTRACT
PUBLIC_FORCE_COUNTER_FEATURES = candidate40.PUBLIC_FORCE_COUNTER_FEATURES

VERIFY_STEPS = candidate40.VERIFY_STEPS
RAMP_DENOMINATOR = 15.0
SUPERVISOR_CONTRACT = "public_linear_arm_authority_ramp_v1"
ASSIGNMENT_CONTRACT = (
    "candidate41_public_arm_ramp_sha256_rank_exact_balanced_complement_v1"
)
ASSIGNMENT_SALT = "pick_tool_candidate41_public_arm_ramp_development_20260722_v1"

PublicStableState = candidate40.PublicStableState
public_stable_state = candidate40.public_stable_state


@dataclass(frozen=True)
class PublicArmRampState:
    """Persistent assignment-invariant shadow clock for stable env slots."""

    activated: torch.Tensor
    stable_count: torch.Tensor
    ramp_scale_previous: torch.Tensor
    ever_positive_authority: torch.Tensor
    ever_full_authority: torch.Tensor


@dataclass(frozen=True)
class PublicArmRampStep:
    """Requested action, next state, and auditable public routing telemetry."""
    ...
    positive_authority_this_action: torch.Tensor
    full_authority_this_action: torch.Tensor
    verification_complete: torch.Tensor
    newly_positive_authority: torch.Tensor
    newly_full_authority: torch.Tensor
    relock: torch.Tensor
    reramp: torch.Tensor
...
def exact_balanced_treatment_mask(
    *, seed: int, num_envs: int, replicate: str
    ...
    """Return the sealed Candidate41 assignment; replicate B complements A."""
...
def apply_public_arm_ramp(
    ...
    *,
    ...
    """Apply the preregistered float32 ``count_before / 15`` arm ramp."""
    ...
    ramp_scale = torch.where(
        public.stable,
        count_before.to(dtype=torch.float32) / denominator,
