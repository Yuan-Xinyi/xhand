#!/usr/bin/env python3
"""Collect Candidate42's public-arm-ramp development A/B artifacts.

Both assignments execute one exact common Candidate39 route.  Candidate42
...
import candidate42_public_arm_ramp as verifier
...
EXTRA_SOURCE_FILES = tuple(
    sorted(
        ...
        | {
            ...
            # this edge from its source closure; Candidate42 preregisters it.
            "scripts/flashsac/collect_candidate39_episode_residual_ab.py",
            "scripts/flashsac/candidate39_episode_residual.py",
            # Candidate42's supervisor and artifact contract reuse these
            ...
            # Candidate42 aliases this exact sealed scientific supervisor by
...
CANDIDATE42_INVOCATION_SEQUENCE = (
    (346, "b", 8),
    (347, "a", 64),
    (347, "b", 64),
    (348, "b", 64),
...
CANDIDATE42_RUN_ORDER = tuple(
    (seed, replicate) for seed, replicate, num_envs in CANDIDATE42_INVOCATION_SEQUENCE
    if num_envs == 64
)
...
PREREGISTRATION_TAG = "pick-tool-candidate42-transactional-arm-ramp-plan-v1-20260722"
COLLECTION_SEAL_TAG = "pick-tool-candidate42-transactional-arm-ramp-validation-v1-20260722"
...
def _require_exact(name: str, actual: Any, expected: Any) -> None:
    ...
    if not valid:
        raise ValueError(f"Candidate42 requires exact {name}={expected!r}")
...
def reconstruct_public_arm_ramp(
    *,
    ...
    """Rebuild Candidate42's public clock and effective arm multiplier.
...
def validate_collection_seal_name_only_diff(
    ...
    if completed.returncode != 0:
        raise ValueError("cannot audit Candidate42 implementation-to-seal diff")
    names = tuple(line for line in completed.stdout.splitlines() if line)
    if names != (artifact_contract.VALIDATION_PLAN,):
        raise ValueError(
            "Candidate42 collection seal may differ from implementation only at the plan"
        )
...
def _protected_plan_contract(plan: Mapping[str, Any], *, pristine: bool) -> dict[str, Any]:
    ...
    try:
        ...
        raise ValueError("Candidate42 plan is not strict JSON") from error
    # Candidate42's preregistered mutation whitelist does not permit changing
    ...
    if strict.get("status") != expected_status:
        raise ValueError("Candidate42 plan status is inconsistent with its seal phase")
    receipt = strict.get("preregistration_receipt")
    if not isinstance(receipt, dict):
        raise ValueError("Candidate42 plan omitted its preregistration receipt")
    if pristine and (
        receipt.get("commit") is not None or receipt.get("sha256") is not None
    ):
        raise ValueError("pristine Candidate42 plan already contained post-hoc receipts")
    if pristine:
        if strict.get("implementation_seal") is not None or strict.get(
            "collection_seal"
        ) is not None:
            raise ValueError("pristine Candidate42 plan already contained a later seal")
    else:
        if not isinstance(strict.get("implementation_seal"), dict) or not isinstance(
            strict.get("collection_seal"), dict
        ):
            raise ValueError("final Candidate42 plan omitted a required seal")
...
def validate_preregistration_protected_fields(
    ...
    if final != pristine:
        raise ValueError("Candidate42 final plan changed a preregistered protected field")
...
def require_exact_collection_head(root: Path, plan: Mapping[str, Any]) -> str:
    ...
    if not isinstance(collection, Mapping) or not isinstance(
        ...
        raise ValueError("Candidate42 plan omitted collection/preregistration authority")
    ...
    if preregistration.get("tag") != PREREGISTRATION_TAG:
        raise ValueError("Candidate42 preregistration tag changed")
    ...
    if head != collection_commit:
        raise ValueError("HEAD must equal the separately sealed Candidate42 collection commit")
    declared_preregistration = preregistration.get("commit")
    if declared_preregistration != preregistration_commit:
        raise ValueError("Candidate42 preregistration receipt disagrees with its tag")
    ...
    if not isinstance(pristine_sha, str) or len(pristine_sha) != 64:
        raise ValueError("Candidate42 pristine plan SHA256 is absent")
    if _git_blob_sha256(
        root, commit=preregistration_commit, relative=artifact_contract.VALIDATION_PLAN
    ) != pristine_sha:
        raise ValueError("Candidate42 pristine preregistration plan changed")
    try:
        ...
        raise ValueError("Candidate42 pristine preregistration plan is not JSON") from error
    if not isinstance(pristine_plan, dict):
        raise ValueError("Candidate42 pristine preregistration plan is not an object")
...
def validate_all_submodules_clean(root: Path) -> None:
    ...
    if superproject.returncode != 0 or superproject.stdout.strip():
        raise ValueError("Candidate42 requires a clean superproject worktree")
    ...
    if status.returncode != 0:
        raise ValueError("cannot audit Candidate42 recursive submodules")
    for line in status.stdout.splitlines():
        if line and line[0] != " ":
            raise ValueError("Candidate42 submodule is missing or at an unregistered commit")
    ...
    if dirty.returncode != 0:
        raise ValueError("Candidate42 requires every recursive submodule to be clean")
...
def _validate_smoke_prerequisite(root: Path) -> tuple[str, str]:
    ...
    artifact_path = _regular_file(
        ...
        label="canonical Candidate42 smoke artifact",
    )
    report_path = _regular_file(
        root / artifact_contract.SMOKE_REPORT_PATH,
        label="canonical Candidate42 smoke report",
    )
    ...
    try:
        ...
        raise ValueError("cannot load the canonical Candidate42 smoke pair") from error
    ...
    if checked_report["artifact_sha256"] != artifact_sha:
        raise ValueError("Candidate42 smoke report does not bind its artifact bytes")
    if Path(checked_report["artifact_output"]).resolve() != artifact_path.resolve():
        raise ValueError("Candidate42 smoke report points to a non-canonical artifact")
    meta, episodes = checked["metadata"], checked["episodes"]
    if (meta["seed"], meta["replicate"], meta["num_envs"]) != (346, "b", 8):
        raise ValueError("Candidate42 smoke run identity changed")
    if meta["runtime_asset_sha256"] != RUNTIME_ASSET_EXPECTED_SHA256:
        raise ValueError("Candidate42 smoke runtime assets changed")
    treatment_eligible = episodes["treatment"] & episodes["eligible"]
    if not bool(treatment_eligible.any()):
        raise ValueError("Candidate42 smoke did not exercise a treatment overlay")
    if not bool((episodes["partial_authority_rows"][treatment_eligible] > 0).any()):
        raise ValueError("Candidate42 smoke did not exercise partial arm authority")
    if bool(episodes["new_abs_action_ge_0999"].any()):
        raise ValueError("Candidate42 smoke failed an action-algebra prerequisite")
...
def _validate_causal_receipts(root: Path) -> dict[str, str]:
    receipts = (
        ...
        (artifact_contract.CANDIDATE41_PLAN, artifact_contract.CANDIDATE41_PLAN_SHA256),
        (artifact_contract.CANDIDATE41_RESULT, artifact_contract.CANDIDATE41_RESULT_SHA256),
        (
    ...
    candidate41_plan = loaded[artifact_contract.CANDIDATE41_PLAN]
    candidate41_result = loaded[artifact_contract.CANDIDATE41_RESULT]
    ...
    if (
        candidate41_plan.get("kind")
        != "pick_tool_candidate41_public_arm_ramp_development_plan_v1"
        or candidate41_plan.get("status") != "sealed_before_smoke_and_collection"
    ):
        raise ValueError("Candidate41 plan semantic identity changed")
    if (
        candidate41_result.get("kind")
        != "pick_tool_candidate41_public_arm_ramp_validation_result_v1"
        or candidate41_result.get("status")
        != "sealed_rejected_at_smoke_before_development"
        or candidate41_result.get("decision", {}).get("candidate41")
        != "reject_without_scientific_effect_claim"
    ):
        raise ValueError("Candidate41 rejection semantic identity changed")
...
def _canonical_invocation_stem(
    ...
    if (seed, replicate, num_envs) == CANDIDATE42_INVOCATION_SEQUENCE[0]:
...
def _validate_exact_invocation_attempt(
    ...
    *,
    ...
    if identity not in CANDIDATE42_INVOCATION_SEQUENCE:
        raise ValueError("invocation is outside Candidate42's exact five-entry sequence")
    current_index = CANDIDATE42_INVOCATION_SEQUENCE.index(identity)
    for index, (item_seed, item_replicate, item_envs) in enumerate(
        ...
        if index < current_index:
            if owned != (True, True, True):
                raise ValueError(
                    f"Candidate42 invocation prerequisite is incomplete: "
                    f"{item_seed}{item_replicate}"
                )
        elif index == current_index:
            if any(owned) and not allow_current_canonical_evidence:
                raise FileExistsError(f"Candidate42 canonical output already exists: {stem}")
        elif any(owned):
            raise ValueError(
                f"later Candidate42 invocation already exists out of order: "
                f"{item_seed}{item_replicate}"
            )
...
def _validate_prior_authority(
    ...
    *,
    ...
    if metadata.get("source_sha256") != dict(source_sha256):
        raise ValueError("prior Candidate42 artifact used a different source authority")
    if (
        metadata.get("smoke_artifact_sha256") != smoke_artifact_sha256
        or metadata.get("smoke_report_sha256") != smoke_report_sha256
    ):
        raise ValueError("prior Candidate42 artifact used different smoke authority")
    git = metadata.get("git")
    if not isinstance(git, Mapping) or git.get("commit") != collection_commit:
        raise ValueError("prior Candidate42 artifact used a different collection commit")
    if metadata.get("runtime_asset_sha256") != RUNTIME_ASSET_EXPECTED_SHA256:
        raise ValueError("prior Candidate42 artifact used different runtime assets")
...
def _validate_published_pair(
    ...
    *,
    ...
    if checked_report["artifact_sha256"] != digest or Path(
        ...
        raise ValueError("prior Candidate42 report does not bind canonical artifact bytes")
    ...
    if (metadata["seed"], metadata["replicate"], metadata["num_envs"]) != (
        ...
        raise ValueError("prior Candidate42 development run identity changed")
...
def _validate_run_order(
    ...
    *,
    ...
    if identity not in CANDIDATE42_RUN_ORDER:
        raise ValueError("run is outside the sealed Candidate42 development order")
    index = CANDIDATE42_RUN_ORDER.index(identity)
    ...
    for prior_seed, prior_replicate in CANDIDATE42_RUN_ORDER[:index]:
        prior_stem = _canonical_dev_stem(root, prior_seed, prior_replicate)
        try:
            artifact_path = _regular_file(
                Path(f"{prior_stem}.pt"),
                label=f"prior Candidate42 artifact {prior_seed}{prior_replicate}",
            )
            report_path = _regular_file(
                Path(f"{prior_stem}.json"),
                label=f"prior Candidate42 report {prior_seed}{prior_replicate}",
            )
        except FileNotFoundError as error:
            raise ValueError(
                f"Candidate42 run order prerequisite is missing: {prior_seed}{prior_replicate}"
            ) from error
    ...
    for later_seed, later_replicate in CANDIDATE42_RUN_ORDER[index + 1 :]:
        ...
        if any(
            ...
            raise ValueError(
                f"later Candidate42 output already exists out of order: {later_seed}{later_replicate}"
            )
...
def validate_runtime_source_count(
    ...
    try:
        ...
        raise ValueError("Candidate42 seal omitted the runtime source count") from error
    if type(declared) is not int or declared != len(source_sha256):
        raise ValueError(
            "Candidate42 static-audit runtime source count does not match "
...
def build_spec(
    ...
    if args.num_envs == 8 and (args.seed, args.replicate) != (346, "b"):
        raise ValueError("the only Candidate42 smoke is exact run 346b/8-env")
    if args.num_envs == 64 and args.seed not in {347, 348}:
        raise ValueError("Candidate42 development uses only seeds 347 and 348")
    ...
    if getattr(args, "kit_args", None) != artifact_contract.KIT_ARGS:
        raise ValueError(
            f"Candidate42 requires exact --kit_args={artifact_contract.KIT_ARGS!r}"
        )
    ...
    for label, (actual, expected) in canonical_inputs.items():
        if actual.resolve() != expected.resolve():
            raise ValueError(f"Candidate42 {label} is not the registered canonical path")
    ...
    if checkpoint_sha256["v6"] != EXPECTED_V6_SHA256:
        raise ValueError("Candidate42 requires the exact sealed V6 checkpoint")
    if checkpoint_sha256["search"] != artifact_contract.SEARCH_CHECKPOINT_SHA256:
        raise ValueError("Candidate42 requires the exact sealed SEARCH checkpoint")
    if checkpoint_sha256["fixed_direction"] != artifact_contract.FIXED_DIRECTION_SHA256:
        raise ValueError("Candidate42 requires the exact sealed fixed_z payload")
    ...
    if (
        ...
        raise ValueError(
            "Candidate42 pre-launch Git authority is dirty, detached, or changed"
        )
    ...
    if not torch.equal(treatment, expected_from_rank):
        raise RuntimeError("Candidate42 assignment mask disagrees with assignment rank")
    if int(treatment.sum()) != args.num_envs // 2:
        raise RuntimeError("Candidate42 assignment is not exactly balanced")
...
def _current_hashes(spec: CollectionSpec) -> dict[str, Mapping[str, str] | str]:
    ...
    for relative in (
        ...
        artifact_contract.CANDIDATE41_PLAN,
        artifact_contract.CANDIDATE41_RESULT,
        artifact_contract.FIXED_DIRECTION_MANIFEST,
    ):
...
def validate_post_application_authority(spec: CollectionSpec) -> None:
    ...
    if source != dict(spec.source_sha256) or _current_hashes(spec) != dict(
        ...
        raise RuntimeError("Candidate42 source or checkpoint changed after collection")
    if runtime_assets != RUNTIME_ASSET_EXPECTED_SHA256:
        raise RuntimeError("Candidate42 runtime asset changed after collection")
    git_paths = tuple(
        sorted(
            set(source)
    ...
    if (
        ...
        raise RuntimeError("Candidate42 Git authority changed after collection")
    sealed_plan = artifact_contract.validate_sealed_plan()
    preregistration_receipt = sealed_plan.get("preregistration_receipt")
    if not isinstance(preregistration_receipt, Mapping):
        raise RuntimeError("Candidate42 sealed plan lost preregistration authority")
    ...
    if (
        ...
        raise RuntimeError(
            "Candidate42 preregistration tag or sealed receipt changed after collection"
        )
...
def run_collection(
    ...
    *,
    ...
    if device.type != "cuda" or not torch.cuda.is_available():
        raise RuntimeError(f"Candidate42 collection requires CUDA, got {device}")
    ...
    if not torch.equal(
        ...
        raise RuntimeError("pre-launch Candidate42 assignment changed")
    if verifier.assignment_mask_sha256(spec.treatment) != spec.assignment_mask_sha256:
        raise RuntimeError("pre-launch Candidate42 assignment receipt changed")
    ...
    if not torch.equal(fixed_payload["fixed_z"], spec.fixed_z):
        raise RuntimeError("pre-launch Candidate42 fixed_z changed")
    ...
    if source_before != dict(spec.source_sha256):
        raise RuntimeError("Candidate42 source authority changed after pre-launch freeze")
    ...
    if git_before["source_files_dirty"] or git_before["flashsac_dirty"]:
        raise RuntimeError("Candidate42 collection requires committed clean sources")
    if (
        ...
        raise RuntimeError("Candidate42 Git authority changed after pre-launch freeze")
    ...
    try:
        ...
        if bool(initial_truth.grasped.any()):
            raise RuntimeError("Candidate42 reset unexpectedly begins grasped")
        ...
        while not tracker.complete and vector_steps < artifact_contract.MAX_EPISODE_ACTIONS:
            ...
            if bool((trigger & triggered).any()):
                raise RuntimeError("a Candidate42 slot triggered twice")
            triggered |= trigger
            ...
            if not torch.equal(residual.eligible, window.active):
                raise RuntimeError("fixed residual assignment incorrectly used Candidate42 arm")
            ...
            if executed is None or not torch.equal(executed, requested_action):
                raise RuntimeError("adapter executed a different Candidate42 request")
        ...
        if not tracker.complete or len(tracker.records) != spec.num_envs:
            raise RuntimeError(
                f"Candidate42 completed {len(tracker.records)}/{spec.num_envs} first episodes"
            )
        ...
        if set(records_by_slot) != set(range(spec.num_envs)):
            raise RuntimeError("Candidate42 tracker omitted an environment slot")
        ...
        if any(int(record["slot_episode_index"]) != 0 for record in ordered):
            raise RuntimeError("Candidate42 artifact contains a post-reset episode")
        ...
        if (
            ...
            raise RuntimeError("Candidate42 source/runtime/checkpoint changed during collection")
