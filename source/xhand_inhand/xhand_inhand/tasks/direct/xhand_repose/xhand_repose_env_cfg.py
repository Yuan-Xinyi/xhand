...
class XHandReposeEnvCfg(DirectRLEnvCfg):
    ...
    object_cfg: RigidObjectCfg = RigidObjectCfg(
        ...
        # lowered from z=0.65: the cube was spawning ~0.13 m above the grasp center (~0,0.07,0.52)
        # and free-falling before the fingers could catch it. 0.55 = grasp-center z (0.52) + cube half (0.03).
    ...
    vel_obs_scale = 0.2
    success_tolerance = 0.1  # initial (loose) alignment threshold [rad]
    max_consecutive_success = 0
    av_factor = 0.1
    act_moving_average = 1.0
    force_torque_obs_scale = 10.0

    # success-tolerance curriculum: start at `success_tolerance` and tighten toward 5 deg.
    # Set `tolerance_curriculum_interval = 0` to disable (fixed `success_tolerance`).
    target_success_tolerance = math.radians(5.0)  # final goal: 5 degrees (~0.0873 rad)
    tolerance_curriculum_increment = 0.9  # multiply tolerance by this at each curriculum step
    tolerance_curriculum_interval = 800  # env steps between possible tightenings (0 = off)
    tolerance_curriculum_success_threshold = 5.0  # min smoothed consecutive successes to tighten
