...
def generate_cuboid_urdf_constant_density(
    ...
    """
    # Explicit mass + inertia: the Isaac Lab URDF importer ignores the <density> tag
    # ("No mass specified" warning -> default mass), so compute it here.
    m, ixx, iyy, izz = compute_mass_and_inertia(scale=scale, density=density)
    # NOTE: link name MUST match the handle+head tools ("handle_head") so the Isaac Lab
    # RigidObject GPU view sees a homogeneous body structure across all pool objects --
    # a differently-named body breaks the batched view (CUDA assert, object dropped).
...
def generate_cylinder_urdf_constant_density(
    ...
    radius = diameter / 2
    # link name "handle_head" for a homogeneous body structure across the pool (see above).
    urdf = f"""<?xml version="1.0"?>
<robot name="handle_head">
